...
The motif discovery process begins with a spatial-time dataset in which each time series is associated with a spatial position. Since direct motif discovery over real-valued subsequences is inefficient, the package first applies **z-score normalization** followed by **SAX indexing** [@keogh_exact_2005; @lin_experiencing_2007]. Symbolic representations such as SAX are widely used to reduce the complexity of motif discovery over real-valued time series [@lin_experiencing_2007]. They enable efficient indexing and comparison of subsequences while preserving relevant shape information. SAX transforms numeric subsequences into symbolic words drawn from an alphabet of size $a$, producing a representation more suitable for frequent-pattern discovery.
